...
ftplib::ftplib()
{
	...
	mp_ftphandle->buf = static_cast<char *>(g_os_malloc(FTPLIB_BUFSIZ));
	if (mp_ftphandle->buf == NULL)
	{
		perror("calloc");
		g_os_free(mp_ftphandle);
	}
	#ifndef NOSSL
...
int ftplib::FtpOpenPort(ftphandle *nControl, ftphandle **nData, transfermode mode, int dir, char *cmd)
{
  ...
  sprint_rest(buf, mp_ftphandle->offset);
	...
	if ((mode == 'A') && ((ctrl->buf = static_cast<char*>(g_os_malloc(FTPLIB_BUFSIZ))) == NULL))
	{
		perror("calloc");
		net_close(sData);
		g_os_free(ctrl);
...
int ftplib::FtpClose(ftphandle *nData)
{
	...
	if (nData->buf) g_os_free(nData->buf);
...
int ftplib::FtpXfer(const char *localfile, const char *path, ftphandle *nControl, accesstype type, transfermode mode)
{
	...
	dbuf = static_cast<char*>(g_os_malloc(FTPLIB_BUFSIZ));
